...
// 				fn.clearWorkflowConfig();
				...
				if(workflowInfo.processDefinitions.length == 0){
					...
					//查找约定的form
				if((Ext.isEmpty(workflowInfo.form)) && (typeof(editForm)!='undefined')){
					var useDefForm=true;
				...
				if(Ext.isEmpty(workflowInfo.form)){
					...
					if(!canBeNoFormFlag){
						...
						if (rs.length == 0) {
							...
							sofa.alert(ruleText["rule4"] || "请先选择数据！");
							return false;
						} 
...
//						workflowInfo.optObject.click();
	 					//兼容resource1.0和1.1的版本 lenglinyong
	 					(workflowInfo.optObject.onClick || workflowInfo.optObject.click)();
	 				}
				});
				return false;
			});
 		...
 		setFormConfig:function(obj){
 			...
 			var fieldConfig = this._taskConfig.formConfig || null;
 			if(fieldConfig == null){
 				return false;
 			}
 			...
 			form.items.each( function( item ){
 				...
 				//20150304 @lenglinyong 
				if(item.readOnly && !(item instanceof Ext.form.Hidden)){
					if(""==item.getValue()){
 		...
 		submitWorkflowForm : function(){
 			if(this._form){
 				...
 				var wfForm=this._form;
// 				this._form.on('beforesubmit', function(){
// 					Ext.getCmp('_wf_tempSaveButton').setDisabled(true);
// 					Ext.getCmp('_wf_commitButton').setDisabled(true);
// 				});
 				this._form.on("success",function(){
 					Ext.getCmp('_wf_tempSaveButton').setDisabled(true);
 					Ext.getCmp('_wf_commitButton').setDisabled(true);
 					Ext.getCmp('_wf_cancelButton').setText("返回");
 					
 				});
 				this._form.on('error', function(){
 					Ext.getCmp('_wf_tempSaveButton').setDisabled(false);
 					Ext.getCmp('_wf_commitButton').setDisabled(false);
 				});
 			}else{
...
// 			Ext.each()
 		...
 		getWorkflowAreaPanelConfig : function(taskConfig, buttons){
 			...
 			if($._isHistory == false){
				...
				str += '    </tr>';
				str += '    <tr>';
				str += '        <td colspan=2 style="padding: 0px 0px">';
				...
				str += '                    </td>';
				str += '                    <td hight="100">';
				str += '                        <textarea id="_wf_opinion" name="_wf_opinion" type="text"  style="width:100%; height:90%;  border:1"></textarea>';
				str += '                    </td>';
				str += '                </tr>';
				str += '           </table>';
				str += '        </td>';
 	...
 	var AssigneeTreeListView = Ext.extend(Ext.form.TreeListView, {
    	...
    	resetSelectedByOutTransition: function(outTransition){
    		...
    		this.store.dataUrl = $._collaborationURL + "colEngine/confirmTaskUser.ctrl?processDefinitionId=" + $._processDefinitionId + "&outTransition=" + outTransition + "&processInstanceId=" + $._processInstanceId;
    		this.onRefresh();
 ...
 * 填充表单.
 */
 fillFormItems = function(formId, data,fillFormExtendCallback) {
	 if(fillFormExtendCallback){
		 fillFormExtendCallback(data);
	 }
	formId.items.each(function(item) {
		if (item.getXType() == "listview" || item.getXType() == "listbox") {
			setListBoxValue(item, data);
		}else if(item.getXType() == "datefield" ){
//			var dateVal=new Date(data[item.id]).format(item.format);
			var dateVal=Date.parseDate(data[item.id],item.format);
			item.setValue(dateVal);
		} else {
 ...
 /**
  * 填充listView控件的值,自动查找并设置显示的值.
  * @param formItem 控件对象
  * @param fillData 待填充的数据
  */
function setListBoxValue(formItem, fillData) {
	var dataList = formItem.data;
	formItem.setValue(fillData[formItem.id]);
	if (typeof (formItem.multiSelect) == 'undefined'
			|| 'false' == formItem.multiSelect) {
		//单选

		Ext.each(dataList, function(data) {
			if (data[formItem.valueField] == formItem.getValue()) {
				formItem.setRawValue(data[formItem.displayField]);
				return false;
			}
		});
	} else {
		...
		var names = "";
		Ext.each(dataList, function(data) {
			var vals = formItem.getValue().split(",");

			var i = 0;
			for (; i < vals.length; i++) {
				if (data[formItem.valueField] == vals[i]) {
					if (i > 0) {
						names = names + ",";
					}
					names = names + data[formItem.displayField];
				}
			}
		});
		formItem.setRawValue(names);
	}
