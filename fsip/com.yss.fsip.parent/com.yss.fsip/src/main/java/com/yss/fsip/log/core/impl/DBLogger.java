...
public class DBLogger implements FSIPLogger {
    ...
    public void success(Class<?> targetClass, Method targetMethod, Object param) {
        ...
        Function functionAnnotation=param.getClass().getAnnotation(Function.class);
        if(functionAnnotation!=null){
            contextMap.put("function",functionAnnotation.code());
        }
    ...
    private Map<String, String> getBizLogMDCContextMap(Class<?> targetClass, Method targetMethod) {
        ...
        if(classAnnotation!=null){
            if(StringUtil.isNotEmpty(methodAnnotation.appName())){
                map.put(MDCKey.SYMBOLIC_NAME,methodAnnotation.appName());
            }
        }
        return map;
    }
        ...
        try {
            if (param instanceof BaseEntity) {
                try {
                   ...
                   // error(SOFALogger.LOGTYPE_SYS, "转换业务数据JSON失败", e);
                }
                // BUG #105515 sofa批量保存日志失败
                String dataId = getIdFromObject(param);
                dataId = (dataId == null || dataId.trim().equals("")) ? "0" : dataId.trim();
                if (!StringUtil.isNumber(dataId)) {
                    dataId = "9";
                    //error(SOFALogger.LOGTYPE_SYS, "值对象[" + bizData.getClass() + "]的id值[" + dataId + "]为非数字类型，不能保存到业务日志中。");
                }
...
//		                		jsonPro = parameterTypes[0].getName();
//		                	}
                        }
    ...
    private String getKeyValue(Object bizData, String[] keyFields) {
        ...
        for (String keyField : keyFields) {
            try {
                ...
                //error(SOFALogger.LOGTYPE_SYS, "获取业务数据[" + clazz.getName() + "]中关键属性[" + keyField + "]失败", e);
            }
        }
