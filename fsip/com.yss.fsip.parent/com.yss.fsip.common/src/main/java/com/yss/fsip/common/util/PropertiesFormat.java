...
import com.yss.fsip.common.util.io.UnicodeReader;

/**
 * 
 *       针对Properties进行扩展的工具类:进行写属性文件时保持原属性文件的注释及文件内容的顺序. 
 * @author LSP
 *
 */
public class PropertiesFormat {
    ...
    public synchronized void load(File file, String charset) {
        ...
        try {
            ...
            isr = new UnicodeReader(inputStream, charset);
            reader = new BufferedReader(isr);
            lr = new LineReader(reader);
            load0(lr);
        } catch (Exception e) {
            throw new RuntimeException("读取属性文件[" + file.getPath() + "]出错！");
        } finally {
            try {
                if (inputStream != null) {
                    inputStream.close();
    ...
    public synchronized String toString() {
        ...
        buffer.append("#! " + PropertiesFormat.class.getSimpleName() + " " + DateUtil.getCurrent() + "\r\n\r\n");
